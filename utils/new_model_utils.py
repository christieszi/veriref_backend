...
class MistralTokenizer:
    ...
    def convert_ids_to_tokens(
        ...
        if any("ï¿½" in t for t in tokens) and self.is_tekken:
            ...
            # if underlying tokenizeir is sentencepiece, we just add "ï¿½"
...
async def mistral(prompt, stream=False, sampling_params=None,suffix=None, url=mistral_endpoint):

    if suffix:
        url = url.split('dsl_small')[0]+suffix
        payload = prompt
    else:
        payload = {
            "prompt": prompt,
...
tokenizer = MistralTokenizer.from_pretrained("mistralai/Mistral-Small-24B-Instruct-2501")
...
async def ask(messages, max_tokens=200):
    encoded_messages = tokenizer.apply_chat_template(messages)
    input_tokens = TokensPrompt(prompt_token_ids=encoded_messages)

    response = await mistral(input_tokens, sampling_params={"max_tokens": max_tokens, "temperature": 0, "top_p": 1})
